...
def emai2(to_email):
   ...
   # JSON 파일 읽기
    ...
    with open(txt_file_path, 'w', encoding='utf-8') as txt_file:
        for i in range(len(data)):
            if i == 0:
                ...
                # 두 번째 항목
                line = f"한 말: {json.dumps(data[i], ensure_ascii=False)}"
            elif i == 2:
                # 세 번째 항목
                line = f"감정: {json.dumps(data[i], ensure_ascii=False)}"
            else:
                # 그 이후 항목은 그대로 출력
                line = json.dumps(data[i], ensure_ascii=False)
    ...
    print(f"{file_path}를 {txt_file_path}로 변환 완료.")
